...
def display_sidebar_content():
    ...
    # ラジオボタンを表示し、選択結果をsession_stateに格納
    st.session_state.mode = st.sidebar.radio(
        ...
        #　「collapsed」でラベル用の余白ごと完全に消し去り、ボタンだけを表示
        # 「visible」(デフォルト)はラベルを表示、その分のスペースも確保
        # 「hidden」はラベルを非表示にするが、ラベル用のスペースは残る
    
    st.sidebar.write("---") # 区切り線の表示
...
def display_conversation_log():
    ...
    """
    # 会話ログのループ処理
    # アプリが記憶している会話履歴（session_stateの「messages」）を順番に取り出して、画面上に表示していく
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        # get_source_icon関数は、ファイルパスを渡すと、そのファイルの種類に応じたアイコンを返す関数
                        icon = utils.get_source_icon(message['content']['main_file_path'])
                        # 参照元ドキュメントのページ番号が取得できた場合にのみ、ページ番号を表示
                        if "main_page_number" in message["content"]:
                            page_info = f"（ページNo. {message['content']['main_page_number']}）"
                            st.success(f"{message['content']['main_file_path']}{page_info}", icon=icon)
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    page_info = f"（ページNo. {sub_choice['page_number']}）"
                                    st.info(f"{sub_choice['source']}{page_info}", icon=icon)
                                else:
                ...
                # 「社内問い合わせ」の場合の表示処理
                    ...
                    if "file_info_list" in message["content"]:
                        ...
                        for file_info in message["content"]["file_info_list"]:
                            # file_infoが辞書形式（パスとページ番号を持っている）の場合
                            # パスとページ番号の両方を持っている可能性がある
                            if isinstance(file_info, dict):
                                # sourceかpath、どちらの名前でも対応できるようにgetを使う
                                path = file_info.get('source') or file_info.get('path') or "不明なパス"
                                # page_numberかpageどちらの名前でも対応できるようにする
                                page = file_info.get('page_number') or file_info.get('page')
                                if page:
                                    display_text = f"{path}（ページNo. {page}）"
                                else:
                                    display_text = path
                            # 参照元のありかに応じて、適したアイコンを取得
                                icon = utils.get_source_icon(path)

                            # file_infoが文字列形式（パスのみ）の場合
                            else:
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        content = {}
        content["mode"] = ct.ANSWER_MODE_1 # 「検索モードだった」と記録
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        file_path_list = [] # パスを覚える用のリスト
        file_info_list = [] # 表示する文字を入れるリスト

        # LLMが回答生成の参照元として使ったドキュメントの一覧が「context」内のリストの中に入っているため、ループ処理
        # AIが参照した文書(context)を順番に取り出して、画面上に表示していく
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        content["file_info_list"] = file_info_list # ページ番号付きのリストを保存
